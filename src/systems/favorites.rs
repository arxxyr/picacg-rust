...
/// 收藏分页控件标记
#[derive(Component)]
pub struct FavoritesPaginationControl;

/// 收藏上一页按钮
#[derive(Component)]
pub struct FavoritesPrevPageButton;

/// 收藏下一页按钮
#[derive(Component)]
pub struct FavoritesNextPageButton;

/// 收藏页码文本
#[derive(Component)]
pub struct FavoritesPageNumberText;
            ...
            .with_children(|pagination| {
                ...
                pagination
                    .spawn((
                        ...
                        BackgroundColor(if favorites_state.page > 1 {
                            AppColors::PRIMARY
                        } else {
                            AppColors::SECONDARY
                        }),
                ...
                // 页码显示
                pagination.spawn((
                    FavoritesPageNumberText,
                    Text::new(format!(
                        "{} / {}",
                        favorites_state.page, favorites_state.total_pages
                    )),
                    TextFont {
                        font: font.clone(),
                        font_size: 14.0,
                ...
                pagination
                    .spawn((
                        ...
                        BackgroundColor(if favorites_state.page < favorites_state.total_pages {
                            AppColors::PRIMARY
                        } else {
                            AppColors::SECONDARY
                        }),
...
pub fn favorites_pagination_interaction(
    mut commands: Commands,
    prev_query: Query<&Interaction, (Changed<Interaction>, With<FavoritesPrevPageButton>)>,
    next_query: Query<&Interaction, (Changed<Interaction>, With<FavoritesNextPageButton>)>,
    ...
    for interaction in prev_query.iter() {
        if *interaction == Interaction::Pressed && favorites_state.page > 1 {
...
pub fn refresh_favorites_ui(
    favorites_state: Res<FavoritesState>,
    mut page_text_query: Query<&mut Text, With<FavoritesPageNumberText>>,
    mut prev_btn_query: Query<
        &mut BackgroundColor,
        (
            With<FavoritesPrevPageButton>,
            Without<FavoritesNextPageButton>,
        ),
    >,
    mut next_btn_query: Query<
        &mut BackgroundColor,
        (
            With<FavoritesNextPageButton>,
            Without<FavoritesPrevPageButton>,
        ),
    >,
) {
    ...
    for mut text in page_text_query.iter_mut() {
        **text = format!("{} / {}", favorites_state.page, favorites_state.total_pages);
    }

    // 更新上一页按钮状态
    for mut bg_color in prev_btn_query.iter_mut() {
        *bg_color = BackgroundColor(if favorites_state.page > 1 {
            AppColors::PRIMARY
        } else {
            AppColors::SECONDARY
        });
    ...
    for mut bg_color in next_btn_query.iter_mut() {
        *bg_color = BackgroundColor(if favorites_state.page < favorites_state.total_pages {
            AppColors::PRIMARY
        } else {
            AppColors::SECONDARY
        });
