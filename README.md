...
- 分类浏览、搜索（排序 + 分类过滤）、排行榜
- 漫画详情（点赞、收藏、分类/标签跳转）
- 阅读器（单页模式、条漫模式、缩放控制、本地文件优先加载）
- 收藏管理（分页浏览）
- 阅读历史、点赞记录
- 个人资料（自动加载头像）
- 下载管理（并发控制、断点续传、CBZ 打包、移动/打开、独立下载设置、全部更新）
- 右键菜单（漫画卡片右键弹出下载/屏蔽菜单，全局生效）
- 快速下载（右键直接下载，自动获取章节列表，无需进入详情页）
- 内容过滤（按分类/标签/标题屏蔽关键词，繁简转换，设置页面管理，分类快速选择面板）
- 分流通道（6 种通道：直连 / CDN / 日本反代 / 美国反代 / 自定义 IP，API 和图片独立配置）
- 网络诊断（测速 + Ping 测试）
- 检查更新（GitHub Releases API，semver 版本比较）
...
- 瀑布流卡片布局
- VSCode 风格自定义滚动条
- 全局滚轮分发（侧边栏/内容区独立滚动，基于光标位置智能分发）
- 通用分页组件
- 无限滚动加载
- 页面缓存（Display::None/Flex 显隐，页面切换零延迟）
- 侧边栏用户头像（登录后自动加载）
- 下载计数徽章（下载中蓝色 + 排队中橙色，实时更新）
- IME 中文输入法支持
