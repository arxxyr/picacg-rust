...
> 2026-08-31 全面评估 + 集中清偿：可一轮完成的全部完成（见「已完成
> 2026-08-31 第二批」）；剩余条目均为需人工目测验收的专项或外部阻塞。

**专项（数周级，需专门排期 + 目测验收，勿在常规迭代里顺手做）**
...
- [ ] i18n 全量接线：2026-08-31 评估——基础设施完整（`I18n` 三语翻译表），但只有侧边栏接线。全量 = 文案盘点（22 页约 200-400 处）+ 翻译表补全（每 key 三语，繁中/英文需人工审校，错译比没译糟）+ 各页场景函数签名加 `Res<I18n>`。属专项，不宜零敲碎打
...
- ~~虚拟滚动推广到 rankings/favorites~~：分页页面每页数据量有界（20-40 项），无实体数压力，瀑布流分帧已够；虚拟滚动的复杂度只在无限滚动场景值回票价。若未来某页改无限滚动再复用 comics 模式
- ~~completed_episodes 存量回填~~：不需要——读取侧双档判据（Completed 看 episode_orders，进行中看增量标记）已覆盖存量；修复后的写入链路让进行中任务自然积累

### 已完成（2026-08-31 第二批：待办集中清偿）

- [x] 首次启动窗口 = 主屏 78% 居中：无保存几何时窗口 `visible: false` 出生，等 `Monitor` 实体就位后一次性调好再现身（不闪不跳）；有保存几何走原路径；120 帧等不到显示器信息则按默认尺寸兜底显示
- [x] 错误页逃生门（games/fried/comments）：错误分支补「重试」按钮（`ErrorRetryButton` 共享控件，按路由分发重载——games/fried 的自动加载只在 OnEnter 跑，必须显式补发请求）+ 分页控件（翻离坏页）
- [x] 条漫 Phase 2 失败章节横幅：`AllChapterPicturesLoadedEvent.failed_chapters` 带回失败章号，阅读器工具栏下方挂可点击重试的横幅（重拉时成功章走缓存瞬回）
- [x] 列表页异步回填身份校验：ComicsLoadedEvent+category、FavoritesLoadedEvent+page、SearchResultsLoadedEvent+keyword/page（⚠️ 搜索只校验 page——state.keyword 是输入框实时内容，校验它会误杀合法响应）；Rankings 天然按 time_type 分桶存储无需校验
- [x] 下载飞行动画：右键下载后圆点从光标处沿贝塞尔弧线飞向侧边栏下载按钮（smoothstep 缓动 + 36→14px 缩小，`fly_curve` 纯函数有单测；拿不到光标/按钮位置时静默跳过不挡下载）
