...
### ✅ 阶段 6.0: Bevy 0.17.3 框架迁移 (进行中 ~80%)
...
- [x] 漫画列表页面 (`systems/comics.rs`)
  ...
  - 分页控制
  - 自定义滚动条
- [x] 漫画详情页面 (`systems/detail.rs`)
  - 封面、标题、作者、标签
  - 章节列表
  - 点赞/收藏/下载按钮
- [x] 下载管理页面 (`systems/downloads.rs`)
  - 下载任务列表
  - 进度显示
  - 暂停/继续/删除操作
  - 断点续传支持
...
#### 自定义滚动条系统
- [x] `ScrollbarTrack` / `ScrollbarThumb` 组件
- [x] 轨道点击快速跳转
- [x] 滑块拖拽滚动
- [x] 自动计算滑块大小和位置
- [x] DPI 缩放适配
...
- [x] 滚动条轨道点击无响应：添加 `Transform` 组件获得 `GlobalTransform`
- [x] 漫画列表 JSON 解析失败：修复 `ComicsData.comics` 字段嵌套结构
- [x] 封面图不显示：修复 `ComicDoc.thumb` 字段类型
- [x] 下载章节顺序：从倒序改为正序（从第1章开始）
- [x] 浮动标题点击跳转位置错误：Query 遍历顺序不确定，改用固定顺序计算
