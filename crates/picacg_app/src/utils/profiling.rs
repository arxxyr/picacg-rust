...
//! | 设置页「系统耗时追踪」/ `F4` | **谁在卡**：按累计耗时排序的系统 Top N | `--features profiling` 构建 + 开关打开 + **重启** |
//!
...
//!    门控的，不编进来就压根不存在（曾照着"不受 feature 门控"的错误判断实现过
//!    一版，跑出来榜单恒空）。故需 `--features profiling` 构建——**不设默认**，
//!    因为 `bevy/trace` 会推高 release 编译的峰值内存。
