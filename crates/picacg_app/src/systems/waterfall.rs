//! 瀑布式显示系统 - 业务特定类型
//!
//! 包含通用的 `WaterfallState` 状态机与业务特定的页面标记、上下文类型。
...
///
/// # 类型参数
/// - `T`: 页面标记类型，用于区分不同页面的瀑布状态
/// - `C`: 上下文类型，用于存储额外的业务数据（默认为 `()`）
///
/// # 示例
/// ```ignore
/// // 简单用法
    ...
    C: Send + Sync + Default + 'static,
{
    ...
    /// 显示游标（已显示到的位置；取批不做 O(n) 前移，只推进游标）
    next_index: usize,
    /// 字体句柄缓存
    ...
    C: Send + Sync + Default + 'static,
{
    ...
    pub fn needs_precreate(&self) -> bool {
        self.precreate_count > 0 && self.next_index >= self.pending_entities.len()
    }
    ...
    /// 获取下一批要显示的实体（游标推进，无元素前移）
    pub fn take_batch(&mut self) -> Vec<Entity> {
        let end = (self.next_index + self.batch_size).min(self.pending_entities.len());
        let batch = self.pending_entities[self.next_index..end].to_vec();
        self.next_index = end;
        batch
    }
    ...
    pub fn has_pending(&self) -> bool {
        self.next_index < self.pending_entities.len()
    }
