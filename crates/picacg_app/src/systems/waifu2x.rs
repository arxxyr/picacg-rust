    ...
    let root = commands
        ...
        .with_children(|root| {
            ...
            .with_children(|header| {
                header.spawn((
                    Text::new(format!("{} Waifu2x 超分辨率", ICON_WAIFU2X)),
                    TextFont {
                        font: font.clone(),
                        font_size: 18.0,
                        ..default()
            ...
            .with_children(|content| {
                ...
                spawn_path_row(
                    ...
                    &waifu2x_state.executable_path,
                    "请选择 waifu2x-ncnn-vulkan 可执行文件...",
                    PathRowType::Executable,
                );
                ...
                spawn_option_row(
                    ...
                    waifu2x_state.gpu_id,
                    OptionRowType::Gpu,
                );

                // --- 输出格式 ---
                spawn_format_row(content, &font, &waifu2x_state);
                ...
                spawn_path_row(
                    ...
                    &font,
                    "输入目录",
                    &waifu2x_state.input_dir,
                    "请选择要处理的图片目录...",
                    PathRowType::InputDir,
                );
                ...
                spawn_path_row(
                    ...
                    &font,
                    "输出目录",
                    &waifu2x_state.output_dir,
                    "请选择输出目录（留空则输出到输入目录）...",
                    PathRowType::OutputDir,
                );

                // --- 开始按钮 ---
                spawn_start_button(content, &font, &waifu2x_state);

                // --- 进度区域 ---
                spawn_progress_area(content, &font, &waifu2x_state);
        ...
        .with_children(|section| {
            ...
            section
                ...
                .with_children(|row| {
                    ...
                    match row_type {
                        PathRowType::Executable => {
                            btn_entity.insert(Waifu2xSelectExeButton);
                        }
                        PathRowType::InputDir => {
                            btn_entity.insert(Waifu2xSelectInputDirButton);
                        }
                        PathRowType::OutputDir => {
                            btn_entity.insert(Waifu2xSelectOutputDirButton);
                        }
                    }
            ...
            section
                ...
                .with_children(|row| {
                    for &value in values {
                        ...
                        let display = match row_type {
                            OptionRowType::Scale => format!("{}x", value),
                            OptionRowType::Noise => {
                                Waifu2xSettings::noise_level_display(value).to_string()
                            }
                            OptionRowType::Gpu => {
                                Waifu2xSettings::gpu_id_display(value).to_string()
                            }
                        };

                        let mut entity_cmds = row.spawn((
                            Button,
                            Interaction::default(),
                            Node {
                                padding: UiRect::new(
                        ...
                        match row_type {
                            OptionRowType::Scale => {
                                entity_cmds.insert(Waifu2xScaleButton { scale: value });
                            }
                            OptionRowType::Noise => {
                                entity_cmds.insert(Waifu2xNoiseButton { level: value });
                            }
                            OptionRowType::Gpu => {
                                entity_cmds.insert(Waifu2xGpuButton { gpu_id: value });
                            }
                        }
...
/// 创建输出格式选择行
fn spawn_format_row(parent: &mut ChildSpawnerCommands, font: &Handle<Font>, state: &Waifu2xState) {
    parent
        .spawn(Node {
            width: Val::Percent(100.0),
            flex_direction: FlexDirection::Column,
        ...
        .with_children(|section| {
            ...
            section
                ...
                .with_children(|row| {
                    for &fmt in Waifu2xSettings::OUTPUT_FORMATS {
                        let is_selected = fmt == state.output_format;
                        let bg_color = if is_selected {
                            AppColors::PRIMARY
                        } else {
                            AppColors::SECONDARY
                        ...
                        .with_children(|btn| {
                            btn.spawn((
                                Text::new(fmt.to_ascii_uppercase()),
                                TextFont {
                                    font: font.clone(),
                                    font_size: 13.0,
                                    ..default()
...
fn spawn_start_button(
    ...
    let (label, bg_color) = if state.is_processing {
        ("处理中...", AppColors::TEXT_SECONDARY)
    } else {
...
fn spawn_progress_area(
    ...
    parent
        ...
        .with_children(|area| {
            ...
            // 进度条（仅处理中显示）
            if state.is_processing && state.total > 0 {
            ...
            area.spawn((
                ...
                Text::new(if state.current_file.is_empty() {
                    String::new()
                } else {
...
pub fn waifu2x_select_exe_interaction(
    mut interaction_query: Query<
        ...
        (Changed<Interaction>, With<Waifu2xSelectExeButton>),
    >,
...
pub fn waifu2x_scale_interaction(
    ...
    for (interaction, scale_btn, mut bg_color, mut border_color, children) in
        interaction_query.iter_mut()
    {
        let is_selected = scale_btn.scale == state.scale;
        match *interaction {
            Interaction::Pressed => {
            ...
            Interaction::None => {
                update_option_button_style(
                    scale_btn.scale == state.scale,
                    &mut bg_color,
                    &mut border_color,
                    children,
                    &mut text_query,
...
pub fn waifu2x_noise_interaction(
    ...
    for (interaction, noise_btn, mut bg_color, mut border_color, children) in
        interaction_query.iter_mut()
    {
        let is_selected = noise_btn.level == state.noise_level;
        match *interaction {
            Interaction::Pressed => {
                if !state.is_processing {
            ...
            Interaction::None => {
                update_option_button_style(
                    noise_btn.level == state.noise_level,
                    &mut bg_color,
                    &mut border_color,
                    children,
                    &mut text_query,
...
pub fn waifu2x_gpu_interaction(
    ...
    for (interaction, gpu_btn, mut bg_color, mut border_color, children) in
        interaction_query.iter_mut()
    {
        let is_selected = gpu_btn.gpu_id == state.gpu_id;
        match *interaction {
            Interaction::Pressed => {
                if !state.is_processing {
...
pub fn waifu2x_format_interaction(
    ...
    for (interaction, fmt_btn, mut bg_color, mut border_color, children) in
        interaction_query.iter_mut()
    {
        let is_selected = fmt_btn.format == state.output_format;
        match *interaction {
            Interaction::Pressed => {
                if !state.is_processing {
            ...
            Interaction::None => {
                update_option_button_style(
                    fmt_btn.format == state.output_format,
                    &mut bg_color,
                    &mut border_color,
                    children,
                    &mut text_query,
...
pub fn refresh_waifu2x_option_buttons(
    ...
    for (btn, mut bg, mut bc, children) in scale_query.iter_mut() {
        update_option_button_style(
            btn.scale == state.scale,
            &mut bg,
            &mut bc,
            children,
            &mut text_query,
    ...
    for (btn, mut bg, mut bc, children) in noise_query.iter_mut() {
        update_option_button_style(
            btn.level == state.noise_level,
            &mut bg,
            &mut bc,
            children,
            &mut text_query,
    ...
    for (btn, mut bg, mut bc, children) in gpu_query.iter_mut() {
        update_option_button_style(
            btn.gpu_id == state.gpu_id,
            &mut bg,
            &mut bc,
            children,
            &mut text_query,
    ...
    for (btn, mut bg, mut bc, children) in format_query.iter_mut() {
        update_option_button_style(
            btn.format == state.output_format,
            &mut bg,
            &mut bc,
            children,
            &mut text_query,
